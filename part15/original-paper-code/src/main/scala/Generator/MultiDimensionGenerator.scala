...
/**
  * Created by LD on 2017-02-24.
  */
object MultiDimensionGenerator {

  /**
    * This parser class is borrowed from:
    * http://stackoverflow.com/questions/5805496/arithmetic-expression-grammar-and-parser
    * @param constants
    * @param userFcts
    * @param random
    */
  class FormulaParser(val constants: Map[String,Double] = Map(), val userFcts: Map[String,String => Double] = Map(), random: Random = new Random) extends JavaTokenParsers {
    ...
    private val unaryOps: Map[String,Double => Double] = Map(
        "sqrt" -> (sqrt(_)), "abs" -> (abs(_)), "floor" -> (floor(_)), "ceil" -> (ceil(_)), "ln" -> (math.log(_)),  "signum" -> (signum(_))
      )
  ...
  val SequentialPrefix = "src/main/resources/sample/sequential/md/"

  val AkkaPrefix = "src/main/resources/sample/akka/md/"
  ...
  def main(args: Array[String]): Unit = {
    ...
    var totalValue = 0
    val results = Array.ofDim[Int](6,6,6,6)
    val r = Random

    try {
      using(io.Source.fromFile("src/main/resources/GeneratorInputMultiDimension")) { source => {
        for (line <- source.getLines) {
          if (!line.isEmpty) {
            val pw = new PrintWriter(new File(s"${SequentialPrefix}complete-$fileCount"))
            val pwAkka = new PrintWriter(new File(s"${AkkaPrefix}complete-$fileCount"))
            buf.clear()
            bufAkka.clear()
            buf ++= "625 "
            for(a <- 1 to 5; b <- 1 to 5; c <- 1 to 5; d <- 1 to 5) {
              val formulaParser = new FormulaParser(
                constants = Map(
                  ...
                  "c" -> c,
                  "d" -> d))
              results(a)(b)(c)(d) = if(formulaParser.evaluate(line) >= 0) 1 else 0
              buf ++= s"$a $b $c $d ${results(a)(b)(c)(d)} "
              bufAkka ++= s"$a $b $c $d=${results(a)(b)(c)(d)}\n"
            }
            ...
            val pw2 = new PrintWriter(new File(s"${SequentialPrefix}$fileCount"))
            val pwAkka2 = new PrintWriter(new File(s"${AkkaPrefix}$fileCount"))
            buf.clear()
            bufAkka.clear()
            for(i <- 0 until 312) {
              val a = r.nextInt(5) + 1
              ...
              val c = r.nextInt(5) + 1
              val d = r.nextInt(5) + 1
              val sampleValue = results(a)(b)(c)(d)
              if(sampleValue != -1) {
                buf ++= s"$a $b $c $d $sampleValue "
                bufAkka ++= s"$a $b $c $d=$sampleValue\n"
                totalValue += 1
                results(a)(b)(c)(d) = -1
              }
            }
