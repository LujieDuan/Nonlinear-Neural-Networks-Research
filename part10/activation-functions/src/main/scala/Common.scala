...
object Common {
  ...
  /**
    ...
    * @return
    */
  def gaussian(z: DenseVector[Double]): DenseVector[Double] = {
    z.map(x => breeze.numerics.exp(-breeze.numerics.pow(x, 2)))
  }
  ...
  /**
    ...
    * @return
    */
  def gaussian_prime(z: DenseVector[Double]): DenseVector[Double] = {
    z.map(el => -2 * el * breeze.numerics.exp(-breeze.numerics.pow(el, 2)))
  }
  ...
  /**
    ...
    * @return
    */
  def relu(z: DenseVector[Double]): DenseVector[Double] = {
    z.map(x => {
      if(x > 0.0) x else 0.0
    })
  }
  ...
  /**
    ...
    * @return
    */
  def relu_prime(z: DenseVector[Double]): DenseVector[Double] = {
    z.map(el => {
      if (el > 0.0) 1.0 else 0.0
    })
  }
