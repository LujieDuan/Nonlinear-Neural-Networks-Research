...
class NonlinearNN(learning_rate: Double, sizes: Seq[Int], epoch: Int, mini_batch_size: Int,
                  activationFunc: (DenseVector[Double]) => DenseVector[Double], activationPrimeFunc: (DenseVector[Double]) => DenseVector[Double]) {
...
//    println(temp)
  ...
  private def feedForward(input: DenseVector[Double]): DenseVector[Double] = {
    ...
    (biases, weights).zipped.foreach((b, w) => output = activationFunc(DenseVector((0 until w.rows).map(x => sum(layerMultiply(w(x, ::).t, output))).toArray) + b))
  ...
  (Seq[DenseVector[Double]], Seq[DenseMatrix[Seq[(Double, Double)]]]) = {
    ...
    //backward pass
    var delta = cost_derivative(activations(layers), y) * activationPrimeFunc(zs(layers - 1))
